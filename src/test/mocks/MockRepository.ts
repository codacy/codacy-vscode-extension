...
export class MockRepository implements Repository {
  revert(paths: string[]): Promise<void> {
    throw new Error(`Unexpected revert(${paths.join(', ')})`)
  }
  tag(name: string, upstream: string): Promise<void> {
    throw new Error(`Unexpected tag(${name}, ${upstream})`)
  }
  deleteTag(name: string): Promise<void> {
    throw new Error(`Unexpected deleteTag (${name})`)
  }
  ...
  hashObject(data: string): Promise<string> {
    return Promise.reject(new Error(`Unexpected hashObject (${data})`))
  }
  ...
  async deleteBranch(name: string, force?: boolean | undefined): Promise<void> {
    ...
    if (index === -1) {
      throw new Error(`Attempt to delete nonexistent branch ${name} ${force}`)
    }
  ...
  async getBranches(_query: any): Promise<Ref[]> {
    return []
  }
  ...
  async fetch(
    arg0?: string | undefined | FetchOptions,
